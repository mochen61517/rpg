...
const SUPP_ICON = {
  // 维生素
  '维生素D':'☀️','维生素C':'🍋','维生素B':'🅱️','维生素B族':'🅱️','B族':'🅱️',
  '复合维生素':'🟡','综合维生素':'🟡','多种维生素':'🟡','维生素E':'🌻','维生素A':'🥕','维生素K':'🥬','叶酸':'🍃',
  // 矿物质
  '镁':'🧂','钙':'🦴','铁':'🩸','锌':'🔩','葡萄糖酸锌':'🔩','葡萄糖酸锌片':'🔩',
  '硒':'🔘','钾':'🍌',
  // 油脂/心血管
  '鱼油':'🐟','Omega-3':'🐟','辅酶Q10':'❤️','纳豆激酶':'🫘','红曲':'🌾',
  // 关节骨骼
  '氨糖':'🦴','软骨素':'🦴','MSM':'⚪','骨胶原':'🦴',
  // 肠胃
  '益生菌':'🦠','益生元':'🌾','消化酶':'🌀','纤维素':'🌾','膳食纤维':'🌾',
  // 抗炎/草本
  '姜黄饮':'🫚','姜黄奶':'🥛','姜黄素':'🫚','姜黄':'🫚','奶蓟草':'🌿','水飞蓟':'🌿',
  '葡萄籽':'🍇','蔓越莓':'🍒','月见草':'🌼','黑种草籽油':'🖤','南非醉茄':'🍂',
  '人参皂苷':'🌱','人参':'🌱','灵芝':'🍄','冬虫夏草':'🍄','枸杞':'🍒',
  // 中式药材/滋补
  '黄精':'🌿','当归':'🌸','阿胶':'🟫','党参':'🌱','黄芪':'🌾','红枣':'🔴','桂圆':'🟤',
  '桑葚':'🟣','黑枸杞':'🍒','罗汉果':'🟢','菊花':'🌼','金银花':'🌼',
  // 超级食物/粉剂
  '巴西莓粉':'🫐','巴西莓':'🫐','羽衣甘蓝粉':'🥬','羽衣甘蓝':'🥬','奇亚籽':'🌱',
  '甜菜根粉':'🍠','甜菜根':'🍠','电解质粉':'⚡','电解质':'⚡','蛋白粉':'🥛','乳清蛋白':'🥛',
  '植物蛋白':'🌱','胶原蛋白':'💧','胶原蛋白肽':'💧','玻尿酸':'💦','NMN':'🧬',
  // 睡眠/神经
  '褪黑素':'🌙','茶氨酸':'🍵','GABA':'🧠','5-HTP':'🧠',
  // 运动表现
  '肌酸':'⚡','左旋肉碱':'🏃','BCAA':'🥤','谷氨酰胺':'🧬','咖啡因':'☕','黑咖啡':'☕','绿茶':'🍵','抹茶':'🍵',
  // 护眼/美容
  '叶黄素':'👁️','玉米黄质':'👁️','越橘':'🫐','生物素':'💅','烟酰胺':'✨'
};
function suppIcon(x){
  if(x && x.icon) return x.icon;
  if(SUPP_ICON[x.t]) return SUPP_ICON[x.t];
  // 去掉剂量与常见剂型后缀，尝试归一匹配
  const norm = x.t.replace(/[（(].*?[）)]/g,'')
                  .replace(/[\d.]+\s*(mg|g|μg|mcg|iu|ml|粒|片|颗|包|袋|勺|杯)/gi,'')
                  .replace(/(粉|片|粒|颗|胶囊|软糖|饮|奶|口服液|咀嚼片|泡腾片|肠溶片|缓释片|滴剂|喷剂)$/,'')
                  .trim();
  if(norm && SUPP_ICON[norm]) return SUPP_ICON[norm];
  return '💊';
}
...
function flDefaultOpenDate(offsetMonths){
  const d=new Date();
  if(!offsetMonths) offsetMonths=1;
  d.setMonth(d.getMonth()+offsetMonths);
  const p=n=>String(n).padStart(2,'0');
  return d.getFullYear()+'-'+p(d.getMonth()+1)+'-'+p(d.getDate());
}
function setFlOpenPreset(v){
  const dateEl=document.getElementById('flOpenDate'); if(!dateEl) return;
  if(v==='custom') return;
  const map={'1m':1,'3m':3,'1y':12};
  dateEl.value=flDefaultOpenDate(map[v]||1);
}
