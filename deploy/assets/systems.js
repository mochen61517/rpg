...
function ensureLifeCompound(){if(!S.lifeCompound||typeof S.lifeCompound!=='object')S.lifeCompound={logs:[],memories:[]};if(!Array.isArray(S.lifeCompound.logs))S.lifeCompound.logs=[];if(!Array.isArray(S.lifeCompound.memories))S.lifeCompound.memories=[];return S.lifeCompound;}
...
function practiceLogs(key){return ensureLifeCompound().logs.filter(x=>x.key===key);}function practiceNewMinutes(key){return practiceLogs(key).reduce((n,x)=>n+(+x.min||0),0);}function practiceTodayMinutes(key){return practiceLogs(key).filter(x=>x.d===todayStr()).reduce((n,x)=>n+(+x.min||0),0);}function practiceWeekMinutes(key){const start=monday();return practiceLogs(key).filter(x=>x.d>=start&&x.d<=shiftDate(start,6)).reduce((n,x)=>n+(+x.min||0),0);}function practiceStage(min){const stages=[[0,'点火'],[60,'萌芽'],[300,'生根'],[1200,'成形'],[3000,'熟练'],[6000,'质变']];let cur=stages[0],next=null;for(let i=0;i<stages.length;i++){if(min>=stages[i][0])cur=stages[i];else{next=stages[i];break;}}return{n:cur[1],at:cur[0],next};}function practiceDays(key){return new Set(practiceLogs(key).map(x=>x.d)).size;}function lifeVariant(key){const t=LIFE_TRACKS[key];return t.variants[seededIndex(todayStr()+key,t.variants.length)];}
function addLifePractice(key,min){const t=LIFE_TRACKS[key];if(!t)return;min=Math.max(1,+min||5);const d=todayStr(),lc=ensureLifeCompound();lc.logs.push({id:'manual:'+Date.now()+':'+key,key,d,min,src:'quick'});grant(t.a,min,false);touchActivity(d);addHist(t.ic+' '+t.n+'复利 +'+min+' 分钟',min,d);save();renderLifeCompound();render();celebrateTask(t.ic+' 今天又向前了一点 · +'+min+' 分钟');}
function syncLifePracticeFromTask(item,d,min,remove){const key=lifeTrackOfTask(item);if(!key)return;const lc=ensureLifeCompound(),lid='task:'+(item.id||item.t)+':'+d,idx=lc.logs.findIndex(x=>x.id===lid);if(remove){if(idx>=0)lc.logs.splice(idx,1);return;}min=Math.max(1,+min||+item.rec||+item.min||5);const row={id:lid,key,d,min,src:'task'};if(idx>=0)lc.logs[idx]=row;else lc.logs.push(row);}
function saveLifeMemory(){const input=document.getElementById('lifeMemoryInput'),sel=document.getElementById('lifeMemoryTrack');const textv=(input&&input.value||'').trim();if(!textv)return;const lc=ensureLifeCompound();lc.memories.push({id:'mem:'+Date.now(),d:todayStr(),text:textv,key:sel&&sel.value||'life'});if(input)input.value='';save();renderLifeCompound();celebrateTask('✨ 一枚生活碎片已被留下');}
...
function renderLifeCompound(){ensureLifeCompound();const todayActive=Object.keys(LIFE_TRACKS).filter(k=>practiceTodayMinutes(k)>0).length,mems=S.lifeCompound.memories||[],todayMems=mems.filter(x=>x.d===todayStr()).length,prompt=LIFE_PROMPTS[seededIndex(todayStr(),LIFE_PROMPTS.length)];const quick=document.getElementById('lifeBlendBox');if(quick)quick.innerHTML='<div class="lc-head"><div><b>🌱 今日生活复利</b><span>长期价值与当下感受，在同一次行动里发生</span></div><div class="lc-score">'+todayActive+'/5 轨道 · '+todayMems+' 枚碎片</div></div><div class="lc-grid">'+Object.keys(LIFE_TRACKS).map(k=>{const t=LIFE_TRACKS[k],m=practiceTodayMinutes(k);return'<div class="lc-card '+(m?'active':'')+'"><div class="lc-card-top"><b>'+t.ic+' '+t.n+'</b><span>'+(m?('今天 +'+m+'m'):'尚未点火')+'</span></div><p>'+lifeVariant(k)+'</p><div class="lc-actions"><button class="btn xs ghost" onclick="addLifePractice(\''+k+'\',5)">火种 +5</button><button class="btn xs" onclick="addLifePractice(\''+k+'\',15)">稳定 +15</button></div></div>';}).join('')+'</div><div class="lc-memory"><div><b>✨ 今日生活碎片</b><span>'+prompt+'</span></div><div class="lc-memory-row"><select id="lifeMemoryTrack"><option value="life">生活本身</option>'+Object.keys(LIFE_TRACKS).map(k=>'<option value="'+k+'">'+LIFE_TRACKS[k].ic+' '+LIFE_TRACKS[k].n+'</option>').join('')+'</select><input id="lifeMemoryInput" maxlength="120" placeholder="一句话就够了…"><button class="btn sm primary" onclick="saveLifeMemory()">留下这一帧</button></div></div>';const detail=document.getElementById('longPracticeBox');if(detail)detail.innerHTML='<div class="lp-head"><div><b>🌳 长期复利轨道</b><span>不追求每天完美，只让总量持续向前</span></div><div class="lp-chapter">✨ '+lifeChapter(mems.length)+' · '+mems.length+' 枚生活碎片</div></div><div class="lp-grid">'+Object.keys(LIFE_TRACKS).map(k=>{const t=LIFE_TRACKS[k],fresh=practiceNewMinutes(k),total=t.base+fresh,st=practiceStage(fresh),pct=st.next?Math.min(100,(fresh-st.at)/(st.next[0]-st.at)*100):100;return'<div class="lp-card"><div class="lp-title"><b>'+t.ic+' '+t.n+'</b><span>'+st.n+'</span></div><div class="lp-total">'+(total/60).toFixed(1)+'h <small>'+t.unit+'累计</small></div><div class="lp-bar"><i style="width:'+pct+'%"></i></div><div class="lp-meta"><span>本周 '+(practiceWeekMinutes(k)/60).toFixed(1)+'h</span><span>'+practiceDays(k)+' 个投入日</span><span>'+(st.next?('距「'+st.next[1]+'」 '+Math.max(0,st.next[0]-fresh)+'m'):'已进入质变期')+'</span></div></div>';}).join('')+'</div><div class="lp-note">唱歌、阅读和钢琴从本版本起分别累计；原有“精神享受”历史总量继续保留，不虚构拆分。由原任务完成产生的投入会自动同步到对应轨道。</div>';}
function setupLifeCompoundUI(){ensureLifeCompound();if(!document.getElementById('lifeBlendBox')){const p=document.createElement('div');p.className='panel life-compound';p.id='lifeBlendPanel';p.innerHTML='<div id="lifeBlendBox"></div>';document.getElementById('todayDetailCockpit')?.insertAdjacentElement('afterend',p);}if(!document.getElementById('longPracticeBox')){const p=document.createElement('div');p.className='panel long-practice';p.id='longPracticePanel';p.innerHTML='<div id="longPracticeBox"></div>';const xp=document.querySelector('#page-growth .xp-ledger');xp?.insertAdjacentElement('afterend',p);}renderLifeCompound();}
